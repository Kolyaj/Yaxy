...
function Yaxy(port, proxy) {
    if (!(this instanceof Yaxy)) {
        return new Yaxy(port, proxy);
    }
    this._listeners = [];
    this._proxy = proxy;
...
Yaxy.prototype = {
    ...
    _onRequest: function(req, res) {
        var state = new State(req, res, this._proxy);
        (function next(i) {
...
State.prototype = {
    ...
    doRequest: function() {
        ...
        var requestOptions = {
            host: (!this._proxy ? host : this._proxy.host),
            port: (!this._proxy ? port : this._proxy.port),
            method: this._req.method,
            path: (!this._proxy ? path : protocol + '//' + host + path),
