...
function httpRequest(serverReq, serverRes, url, modifiers) {
    ...
    var requestOptions = {
        ...
        headers: extractHeaders(modifiers, 'setRequestHeader', 'removeRequestHeader', serverReq.headers)
    };
    var clientReq = require('http').request(requestOptions, function(clientRes) {
        serverRes.writeHead(clientRes.statusCode, extractHeaders(modifiers, 'setResponseHeader', 'removeResponseHeader', clientRes.headers));
        ...
        if (exists) {
            require('fs').stat(fname, function(err, stat) {
                ...
                if (stat.isFile()) {
                    ...
                    serverRes.writeHead(200, extractHeaders(modifiers, 'setResponseHeader', 'removeResponseHeader', {
...
function extractHeaders(modifiers, setModifierName, removeModifierName, headers) {
    headers = headers || {};
    modifiers.forEach(function(modifier) {
        if (modifier.name == setModifierName) {
            var headerParts = modifier.value.split(':');
            headers[headerParts.shift().trim()] = headerParts.join(':').trim();
        } else if (modifier.name == removeModifierName) {
            delete headers[modifier.value.trim()];
        }
    });
    return headers;
}