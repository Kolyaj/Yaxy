...
exports.createServer = function(rewriter) {
    ...
    return require('http').createServer(function(serverReq, serverRes) {
        ...
        function doRequest() {
            ...
            } else if (rule.url.indexOf('http://') == 0 || rule.url.indexOf('https://') == 0) {
...
function httpRequest(serverReq, serverRes, url, modifiers) {
    ...
    var requestOptions = {
        host: host,
        port: +(location.port || host.split(':')[1] || (secure ? 443 : 80)),
    ...
    clientReq.on('error', function(err) {
        console.log('Request error: ' + err.message + ' (' + url + ')');
        serverRes.end();
    });
    ...
    if (!serverReq.connection.__yaxy_clientReqs) {
        serverReq.connection.__yaxy_clientReqs = [];
        serverReq.connection.once('close', function() {
            serverReq.connection.__yaxy_clientReqs.forEach(function(req) {
                req.abort();
            })
        });
    }
    serverReq.connection.__yaxy_clientReqs.push(clientReq);
}
...
function buildHeaders(modifiers, isRequest, headers) {
    ...
    modifiers.forEach(function(modifier) {
        ...
        } else if (isRequest && modifier.name == 'setCookie') {
            var cookieParts = modifier.value.split('=');
            headers['cookie'] = setCookie(headers['cookie'], cookieParts[0].trim(), cookieParts[1].trim());
        } else if (isRequest && modifier.name == 'removeCookie') {
            headers['cookie'] = removeCookie(headers['cookie'], modifier.value);
            if (!headers['cookie']) {
...
function setCookie(header, name, value) {
    header = header || '';
    header = removeCookie(header, name);
    if (header) {
        header += '; ';
    }
    return header + encodeURIComponent(name) + '=' + encodeURIComponent(value);
}

function removeCookie(header, name) {
    return header.split('; ').map(function(cookie) {
        return cookie.trim();
    }).filter(function(cookie) {
        return cookie.indexOf(encodeURIComponent(name) + '=') != 0;
    }).join('; ');
}
